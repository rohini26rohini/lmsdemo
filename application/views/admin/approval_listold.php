            ...
            <table id="syllabus_data" class="table table-striped table-sm" style="width:100%">
                ...
                ?>
                    <tr id="row_<?php echo $approve['leave_id'];?>">
                        ...
                        <!-- </td> -->
                        <td>
                        <?php if ($approve['leave_status']=="0") {
                                echo '<span class="pending">Pending</span>';
                            }else if($approve['leave_status']=="1") {
                                echo '<span class="admitted">Approved</span>';
                            }else if($approve['leave_status']== "2") { 
                                echo '<span class="declined">Rejected</span>';
                            }   
                        ?>  
                        </td>
                        <!-- <td>
                            <button class="btn btn-default option_btn "  onclick="view_leave(<?php echo $approve['leave_id'];?>)">
                                <i class="fa fa-eye "></i>
                            </button>
                        </td> -->
                        <td nowrap>
                            ...
                            <?php  if ($approve['leave_status']=="0") { ?>
                                <select name="status" class="form-control tableSelect c_status" onchange="edit_leave_status('<?php echo $approve['leave_id'];?>');" id="id_<?php echo $approve['leave_id'];?>">
                                    <option value="0" >Pending</option>
                                    <?php if ($approve['leave_status']=="0") { ?>
                                        <option value="1" >Approved</option>
                                        <option value="2">Rejected</option>
                                    <?php } elseif( $approve['leave_status']=="1") {?>
                                        <option value="1">Approved</option>
                                    <?php } ?>
                                </select>
                            <?php } ?>
