...
function createNewTextBox() {
    $.ajax({
        type: "POST",
        url: '<?php echo base_url();?>backoffice/Transport/get_payment_head',
        ...
        success: function(result) {
            ...
            markup += '<td data-title="Fee head">'+ result1 +'</td>';
            markup += '<td data-title="Fee amount"><input class="form-control numberswithdecimal" type="text" name="feeamount[' + counter + ']" id="feeamount' + counter + '"/></td>';
...
$("form#add_feedef_form").validate({       //route
        rules: {
            title: {required: true,
                remote: {
                    url: '<?php echo base_url();?>backoffice/Transport/titleCheck',
                    type: 'POST',
                    data: {
                        title: function() {
                          return $("#title").val();
        ...
        submitHandler: function(form) {
            $(".loader").show();
            $.ajax({
                url: '<?php echo base_url();?>backoffice/Transport/fee_def_add',
                type: 'POST',
                ...
                success: function(response) {
                    ...
                    if(obj.st == 1){
                        ...
                        $.ajax({
                            url: '<?php echo base_url();?>backoffice/Transport/load_fee_def_ajax',
                            type: 'POST',
                            data: {
                                <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'
                            },
        ...
        $.confirm({
            ...
            buttons: {
                'confirm': {
                    ...
                    btnClass: 'btn-blue',
                    action: function() {
                        $.post('<?php echo base_url();?>backoffice/Transport/edit_fee_status/', {
                            id: id,st: st,
                            <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'
                        }, function(data) {
...
function edit_feedef(id) {
    ...
    $(".loader").show();
    $.ajax({
        url: '<?php echo base_url();?>backoffice/Transport/get_feedef_by_id',
        type: 'POST',
        data:{edit_id:id,
              <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'},
        success: function(response) {
...
$("form#edit_fee_form").validate({       //route
        ...
        submitHandler: function(form) {
            $(".loader").show();
            $.ajax({
                url: '<?php echo base_url();?>backoffice/Transport/fee_def_edit',
                type: 'POST',
    ...
    function view_feedef(id) {
        $(".loader").show();
        $.ajax({
            url: '<?php echo base_url();?>backoffice/Transport/view_feedef_by_id',
            type: 'POST',
            data:{id:id,
                  <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'},
            success: function(response) {
                ...
                // alert(obj.name);
                 ...
                 var table = "<div class= 'table-responsive table_language'><table class='table table_followup table-sm table-bordered table-striped'>";
                    table += "<tr><th>Fee Head</th><th  style='text-align:right;'>Fee Amount [INR]</th></tr>";
                    table +=obj.html;
                    table += "<table></div>";
