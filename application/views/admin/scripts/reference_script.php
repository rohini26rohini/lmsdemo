<script>
    ...
    //         "ajax": {
    //         "url": "<?php echo base_url();?>backoffice/Commoncontroller/cc_assigned_details_ajax",
    //         "type": "POST",
    //         data:{
    //              filter_cce:$("#filter_cce").val(),filter_name:$("#filter_name").val(),filter_course:$("#filter_course").val(),filter_number:$("#filter_number").val(),filter_status:$("#filter_status").val(),filter_sdate:$("#filter_sdate").val(),filter_edate:$("#filter_edate").val(),filter_enquiry :$("#filter_enquiry").val(),filter_place:$("#filter_place").val(),filter_cce:$("#filter_cce").val(),filter_rand_num:$("#filter_rand_num").val(),
    //             <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'},
    ...
    // } );
        ...
        function load_data(){
            ...
            $(".loader").show();
            $.ajax({
                url:"<?php echo base_url(); ?>backoffice/Commoncontroller/fetch",
                method:"POST",
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response != "2" && response != "0") {
                        ...
                        $.ajax({
                                    url: '<?php echo base_url();?>backoffice/Call_center/load_cc_assigned_ajax',
                                    type: 'POST',
                                        data: {
                                            <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'
                                        },
   ...
   $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(obj, function (i, v) {
                        ...
                        table += "<tr id=row_"+v.followup_id+"><td>"+j+"</td><td>"+v.date+"</td><td>"+status+"</td><td>"+v.comment+"</td><td><a class='btn btn-default option_btn' style='width:auto;' title='Delete' onclick='delete_followup("+v.followup_id+")'><i class='fa fa-trash-o'></i></a></td></tr>";
                    });
            ...
            buttons: {
                'confirm': {
                    ...
                    action: function() {
                        $(".loader").show();
                        $.ajax({
                            url: '<?php echo base_url();?>backoffice/Call_center/enquiry_assign_status_change',
                            type: 'POST',
                            data:{id: id,selectid: $("#edit_list_status_"+id).val(),<?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'},
                            success: function(response) {
                                if (response != "2" && response != "0") {
