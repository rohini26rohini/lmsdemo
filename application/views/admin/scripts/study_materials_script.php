...
<script>
    ...
    $("#filter_batch").change(function(){
        ...
        if (batch != "") {
            $(".loader").show();
            $.ajax({
                url: '<?php echo base_url();?>backoffice/Employee/get_studymaterial_by_batch',
                type: 'POST',
                data: {
                    batch_id: batch,
                    <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'
    ...
    function downloadLearningModule(id){
        $(".loader").show();
        $.ajax({
            url: '<?php echo base_url();?>backoffice/faculty-learning-module/'+id,
            type: 'POST',
            data: {
