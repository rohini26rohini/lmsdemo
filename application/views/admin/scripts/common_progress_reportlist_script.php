...
function get_course(){
   ...
   var centre_id=$("#filter_centre").val();
     if (school_id != "" && centre_id != "") {
            $(".loader").show();
            $.ajax({
                url: '<?php echo base_url();?>backoffice/Commoncontroller/get_course_bySchool',
                type: 'POST',
                data: {
                    school: school_id,
                    centre_id: centre_id,
                ...
                success: function(response) {
                   ...
                   if (course_id != "") 
                   {
                        $(".loader").show();
                        $.ajax({
                            url: '<?php echo base_url();?>backoffice/Commoncontroller/get_batch_byCourse',
                            type: 'POST',
                            data: {
                                course_id: course_id,
                                <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>'
 ...
 //searching
   function search(){
            $('#loadreportview').show();
            $.ajax({
                url: '<?php echo base_url();?>backoffice/Commoncontroller/search_student_progresslist',
                type: "post",
...
$(document).on('click', '.batchaverage', function () { 
    ...
    $(".loader").show();
     if (exam_id != '' && attempt!='') {
            $.ajax({
                url: '<?php echo base_url();?>backoffice/Commoncontroller/get_batch_exam_details',
                type: 'POST',
                data: {
                    exam_id: exam_id,
                    attempt: attempt,
...
$(document).on('click', '.studentindivigualprogress', function () {    
            ...
            $.ajax({
                url: '<?php echo base_url();?>backoffice/Commoncontroller/progress_report',
                type: 'POST',
                data: {
