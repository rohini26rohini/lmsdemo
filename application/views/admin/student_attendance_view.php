                            ...
                            <table class="table table-bordered table-sm table_exam_details">
                                <tbody>
                                    <tr>
                                        <th>Date</th>
<!--                                        <th>Module</th>-->
                                        <th colspan="10" class="align-center">Sessions</th>
                                    </tr>
                                    <?php
                                    if(!empty($attendance)) { 
                                        foreach($attendance as $mark) { //print_r($mark);
                                            $dayschedule = $this->common->get_student_attendance($batch->batch_id, $mark->schedule_date);
                                    ?>
                                    <tr>
                                        <td  class="text-center clatime"><?php echo  date('d-M-Y', strtotime($mark->schedule_date));?></td>
<!--
...
<ul class="dir_attendance-session dir-vattendanc">
                                                    <?php 
                                                        if(!empty($dayschedule)) {
                                                            foreach($dayschedule as $schedule){
                                                                $studentattendance = $this->common->getstudent_attendance($schedule->schedule_id, $student_id);
...
<!--                                        <th>Module</th>-->
                                    ...
                                    ?>
                                    <tr>
                                        <td class="text-center clatime2"><?php echo  date('d-M-Y', strtotime($mark->start_date_time));?></td>
<!--
                                        <td>Lorem Ipsum is simply dummy text of the printing and typesetting
                                            industry. </td>
-->
                                        ...
                                        <ul class="dir_attendance-session dir-vattendanc">
                                                <?php 
                                                        if(!empty($dayschedule)) {
                                                            foreach($dayschedule as $schedule){ 
                                                                $studentattendance = $this->common->getstudent_attendance($schedule->id, $student_id,'exam');
                                                                //print_r($studentattendance);
                                                    ?>
