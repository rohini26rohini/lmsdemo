        ...
        if($_POST){
            ...
            if($staff_personal_exist == 0){ 
                ...
                if($res > 0){ 
                    ...
                    logcreator('insert', 'database', $who, $what, $table_row_id, 'am_staff_personal', 'New staff details added');
...
*/
        ...
        if($_POST)
        {
           ...
           if($staff_personal_exist <= 1)
           {
                ...
                if($res)
                {
                    ...
                    $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
                    logcreator('update', 'database', $who, $what, $id, 'am_staff_personal', 'Staff detailed edited');
    ...
    public function staff_personals_edit()
    {
            ...
            if($res=1){
                $what=$this->db->last_query();
                $who=$this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
                logcreator('update', 'database', $who, $what, $id, 'am_staff_personal', 'Staff details edited');
            }
            print_r($res);
    ...
    public function delete_staff_personals()
    {
        ...
        if($res=1){
            $what = $this->db->last_query();
            $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
            logcreator('delete', 'database', $who, $what, $id, 'am_staff_personal', 'Staff detailed removed');
        }
        print_r($res);     
    }
     ...
     function get_modules_by_subject($subject_id = NULL)
    {
        ...
        $modules = $this->db->get_where('mm_subjects', array('parent_subject' => $subject_id,'subject_status'=>'1'))->result_array();
    ...
    public function staff_education_add(){       
        ...
        foreach($categoryArr as $category){
            ...
            if(isset($specificationArr[$i])){
                if($category =='Others' && $education_id[$i]!=''){
                    $exist = array();
                }else {
                    if(isset($education_id[$i])){
                        $exist = $this->staff_enrollment_model->check_specification($edit_personal_id,$categoryArr[$i]);                  
                    } 
                }
                if(!empty($exist)){ 
                    ...
                    $data = array(
                        ...
                        'marks'         => !empty($marksArr[$i]) ? $marksArr[$i] : '0.00',
                    ...
                    $result = $this->staff_enrollment_model->staff_education_edit($exist['education_id'],$data);               
        ...
        if($_POST){
            ...
            //if($staff_experience_exist == 0){
                ...
                if($res = 1){
                    $what = $this->db->last_query();
                    $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
                    logcreator('update', 'database', $who, $what, $experience_id, 'am_staff_experience', 'Staff experience detail updated');
                ...
                if($res = 1){
                    ...
                    $table_row_id = $this->db->insert_id();
                    $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
                    logcreator('insert', 'database', $who, $what, $table_row_id, 'am_staff_experience', 'Staff experience details added');
...
public function staff_approvalform() {
    if($_POST){
        $personal_id    = $this->input->post('personal_id');
        if($personal_id!='') {
            $_POST['joining_date'] = date('Y-m-d', strtotime($_POST['joining_date']));
        ...
        if($_POST){
            ...
            if($this->input->post('type') == 2){
                if(empty($this->input->post('start_date')) || empty($this->input->post('end_date'))){
                    $result['status']   =   false;
                    $result['message']  =   'Start date and End date fields are required';
                    $result['data']     =   null; 
                    print_r(json_encode($result));
                    exit;
                }else{
            ...
            $class_timing_empty=0;
            $available_weeks = [];
            $weekdays = ['sun','mon','tue','wed','thu','fri','sat'];
            $weekday_start_time = ['sunday_start_time','monday_start_time','tuesday_start_time','wednesday_start_time','thursday_start_time','friday_start_time','saturday_start_time'];
            $weekday_end_time = ['sunday_end_time','monday_end_time','tuesday_end_time','wednesday_end_time','thursday_end_time','friday_end_time','saturday_end_time'];
            $weekday_times = [[],[],[],[],[],[],[]];
            $f = 0;
            foreach($weekdays as $k=>$day){
                if($this->input->post($day)) { 
                    $available_weeks[$day] = $this->input->post($day);
                    $availability_start_time = $this->input->post($weekday_start_time[$k]);
                    $availability_end_time = $this->input->post($weekday_end_time[$k]);
                    if(empty($availability_start_time[0]) || empty($availability_end_time[0])){
                        $class_timing_empty=1;
                    }else{
                        foreach($availability_start_time as $ki=>$row){
                            array_push($weekday_times[$k],['week_day'=>ucfirst($day),'start_time'=>date('H:i:s',strtotime($row)),'end_time'=>date('H:i:s',strtotime($availability_end_time[$ki]))]);
                            $f = 1;
                        }
                    }
                } else {$available_weeks[$day] = 0;}
            }
            if($f == 0){
                $result['status']   =   false;
                $result['message']  =   'Please select at least on day and time';
                $result['data']     =   null; 
                print_r(json_encode($result));
                exit;
            }
            if($class_timing_empty){
                $result['status']   =   false;
                $result['message']  =   'Some available timings are empty';
                $result['data']     =   null; 
                print_r(json_encode($result));
                exit;
            }
            if($this->input->post('edit_id') == ''){
                $exist = $this->common->check_if_dataExist('am_faculty_availability',array('staff_id'=>$this->input->post('faculty'),'center_id'=>$this->input->post('center_name'),'status!='=>2));
            }else{
                $exist = 0;
            }
            if($this->input->post('type') == 2){
                $exist = 0;   // Need to check date panding
            }
            if($exist ==0){
                $data = array('staff_id'=>$this->input->post('faculty'),
                            'type'=>$this->input->post('type'),
                            'center_id'=>$this->input->post('center_name'),
                            'day_sun'=>$available_weeks['sun'],
                            'day_mon'=>$available_weeks['mon'],
                            'day_tue'=>$available_weeks['tue'],
                            'day_wed'=>$available_weeks['wed'],
                            'day_thu'=>$available_weeks['thu'],
                            'day_fri'=>$available_weeks['fri'],
                            'day_sat'=>$available_weeks['sat'],
                            'date_from'=>$start_date,
                            'date_to'=>$end_date
                            );
                if($this->input->post('edit_id') == ''){
                    $status = $this->Staff_model->save_faculty_availability($data,$weekday_times);
                }else{
                    $status = $this->Staff_model->edit_faculty_availability($data,$weekday_times,$this->input->post('edit_id'));
                }
                if($status){
                    $result['status']   =   true;
        ...
        foreach($data as $d) {
          ...
          else
          {
            ...
            $html .= '<tr>
                        ...
                        <td>'.$availability.'</td>
                        <td>
                            <button class="btn btn-default option_btn" title="View" onclick="view_availability('.$d->avai_id.')">
                                <i class="fa fa-eye"></i>
                            </button>
                            <button class="btn btn-default option_btn" title="Edit" onclick="get_data('.$d->avai_id.')">
                                <i class="fa fa-pencil"></i>
                            </button>
                            <button class="btn btn-default option_btn" title="Delete" onclick="delete_availability('. $d->avai_id.')">
...
public function load_staffList_byAjax()
{
        ...
        $columns_valid = array(
            ...
            "am_staff_personal.joining_date",
        ...
        foreach ($list as $rows) {
            ...
            if($login_status == "1")
            {
               $status='<span class="btn mybutton  mybuttonActive" onclick="edit_staff_status('.$rows['personal_id'].','.$login_status.');">
                               '.$this->lang->line('Active').'</span>'; 
            }
            elseif($login_status == "0")
            {
                $status='<span class="btn mybutton  mybuttonInactive" onclick="edit_staff_status('.$rows['personal_id'].','.$login_status.');">
                               '.$this->lang->line('Inactive').'</span>';
            }

            $no++;
            $time = strtotime($rows['joining_date']);
            $date = ($time === false) ? '0000-00-00' : date('d-m-Y', $time);
            $row   = array();
            $row[] = $no;
            $row[] = $rows['name'];
            ...
            $row[] = $role_name;
            $row[] = $rows['mobile'];
            $row[] = !empty($rows['joining_date']) ? date('d-m-Y',strtotime($rows['joining_date'])) : '&nbsp;';
            $row[] = $status;
            $row[] = '<a href="'.base_url().'backoffice/manage-staff/'.$rows['personal_id'].'" name="personal_id" id="clicker" data-personal="'. $rows['personal_id'].'">
                        <button class="btn btn-default option_btn add_new_btn" title="Edit">
                            <i class="fa fa-pencil "></i>
                        </button>
                        </a>
                        <a class="btn btn-default option_btn  " title="Delete" onclick="delete_staff_personals('. $rows['personal_id'].')">
                            <i class="fa fa-trash-o"></i>
                        </a>';
            $data[] = $row;
        }
...
public function get_staff_documents(){
                    ...
                    foreach($documents as $row){ 
                        ...
                        $html = $html.'</div>
                            <div class="col actions">
                                ...
                                <button class="btn btn-default btn_details_view" title="Delete" onclick="delete_docs(\''.$row->staff_documents_id.'\')">
                                    <i class="fa fa-trash-o"></i>
                                </button>
                            </div>
                        </li>';
 ...
 $exist=$this->common->check_if_dataExist('am_faculty_availability',array('staff_id'=>$this->input->post('faculty'),'center_id'=>$this->input->post('center_name'),'avai_id!='=>$id,'status!='=>2));
            if($exist == 0){
            ...
            if($update) {
                ...
                $what = $this->db->last_query();
                $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
                logcreator('delete', 'database', $who, $what, $id, 'am_faculty_availability', 'Faculty availablity details updated');
    ...
    public function get_faculity_availability_data()
    {
      if($_POST)
      {
         ...
         $data['date_to']=date('d-m-Y',strtotime($data['date_to']));
          ...
          $class_timing=array();
          $class_sessions=$this->common->get_alldata('am_faculty_availability_detail',array("avai_id"=>$this->input->post('id')));
   ...
   //echo "<pre>"; print_r($class_sessions);
    ...
    public function get_faculity_availability_dataView()
    {
      if($_POST)
      {
...
public function delete_faculty()
    {
        ...
        if($res=1){
            $what = $this->db->last_query();
            $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
            logcreator('delete', 'database', $who, $what, $id, 'am_faculty_availability', 'Faculty availability detailed deleted');
        }
        print_r($res);     
    }
        ...
        if($data->num_rows() > 0){
            ...
            foreach($data->result() as $row){
                ...
                $output .= '
                            ...
                            <td>'.$i.'</td>
                            <td> '.$row->name.'</a></td>
							<td> '.$row->registration_number.'</a></td>
                            <td>'.$row->role_name.'</td>
                            <td>'.$row->mobile.'</td>
                            <td>'.$status.'</td>
                            <td>
                                ...
                                <a class="btn btn-default option_btn  " title="Delete" onclick="delete_staff_personals('.$row->personal_id.')">
                                    <i class="fa fa-trash-o"></i>
                                </a>
                            </td>
                            </tr>';
    ...
    { 
        ...
        if($res=1){
            $what = $this->db->last_query();
            $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
            logcreator('delete', 'database', $who, $what, $id, 'am_staff_education', 'Staff education details removed');
        }
        print_r($res); 
        // if($_POST){
                    ...
                    <div id="section_duplicates2">
                        ...
                        <div class="add_wrap">
                            ...
                            $html .= '<input type="hidden" name="files" id="files" value="'.$query["file"].'" />';
                                    if($rows["category"]=="Classx" && $rows["specification"]!=''){
                                $html .= '<div class="col-sm-4">
                                    ...
                                    }else if($rows["category"]=="Classxii" && $rows["specification"]!=''){ 
                                $html .= '<div class="col-sm-4">
                                            <div class="form-group">
                                                <label>+2/VHSE</label>
                                                <input type="hidden" name="category[]" value="'.$rows["category"].'"/>
                                    ...
                                    }else if($rows["category"]=="Degree" && $rows["specification"]!=''){ 
                                $html .= '<div class="col-sm-4">
                                            <div class="form-group">
                                                <label>Degree</label>
                                                <input type="hidden" name="category[]" value="'.$rows["category"].'"/>
                                    ...
                                    }else if($rows["category"]=="PG" && $rows["specification"]!=''){ 
                                $html .= '<div class="col-sm-4">
                                            <div class="form-group">
                                                <label>PG</label>
                                                <input type="hidden" name="category[]" value="'.$rows["category"].'"/>
        ...
        if($data->num_rows() > 0){
            ...
            foreach($data->result() as $row){
                ...
                $output .= '<tr>
                            ...
                            <td>'.$days.'</td>
                            <td><button class="btn btn-default option_btn" title="Edit" onclick="get_data('.$row->avai_id.')"> <i class="fa fa-pencil "></i> </button><button class="btn btn-default option_btn" title="Delete" onclick="delete_availability('.$row->avai_id.')"><i class="fa fa-trash-o"></i></button>
                            </td>
                            </tr>';
                            $i++;
                        }
...
public function edit_staff_status()
{
    ...
    {
        $id=$this->input->post('id');
        $exist=$this->common->check_if_dataExist('am_schedules',array("staff_id"=>$id,"schedule_date>="=>date("Y-m-d"),"class_taken"=>0,"schedule_status"=>1));
        if($exist >0)
        {
            $ajax_response['st']=2; 
        ...
        $response=$this->Common_model->update('am_users_backoffice', array("user_status"=>$status), array("user_id"=>$user_id));
        //echo $this->db->last_query();
        if($response)
        {
            $ajax_response['st']=1; 
   ...
   $html = '<thead> 
        ...
        if(!empty($staffArr)) 
        {
            ...
            foreach($staffArr as $row)
            {
                ...
                if($login_status == "1")
                {
                  $status='<span class="btn mybutton  mybuttonActive" onclick="edit_staff_status('.$row['personal_id'].','.$login_status.');">
                               '.$this->lang->line('Active').'</span>'; 
                }
                elseif($login_status == "0")
                {
                    $status='<span class="btn mybutton  mybuttonInactive" onclick="edit_staff_status('.$row['personal_id'].','.$login_status.');">
                                   '.$this->lang->line('Inactive').'</span>';
                }
                $jdate = $row['joining_date'] != null ? date('d-m-Y',strtotime($row['joining_date'])):'&nbsp;';
                $html .= '
                            <tr>
                            <td>'.$i.'</td>
                            <td> '.$row['name'].'</a></td>
    ...
    public function get_staff_idcard_preview($id){
        // echo $id;
        // $data['staffArr']   = $this->staff_enrollment_model->get_staff_personal_by_id($id);
        // print_r($data['staffArr']); 
        $data['staffArr'] = $this->common->getSingle_roles($id);
        // show($data['staffArr']);
        $html = $this->load->view('admin/staff/staff_id_view',$data,TRUE);
        $returndata['st']=1;
    ...
    public function download_idcard($id){ 
        ...
        $data['staffArr'] = $this->common->getSingle_roles($id);
        $this->db->where('personal_id', $data['staffArr']['personal_id']);
        $this->db->update('am_staff_personal', array('idcard_status'=> 1));
        $html = $this->load->view('admin/pdf/staff_id_pdf.php',$data,TRUE);
        $file = $data['staffArr']['name'].$data['staffArr']['personal_id'].'.pdf';
        $filepath = FCPATH.'uploads/staff_id_card/'.$file;
        $url = base_url('uploads/staff_id_card/'.$file);
    ...
    public function manage_faculty_attendance()
    {
        check_backoffice_permission('manage-faculty-attendance');
        $this->data['page']="admin/staff_attendance";
		$this->data['menu']="staff";
        $this->data['breadcrumb']="Manage Faculty Attendance";
        $this->data['menu_item']="manage-faculty-attendance";
        $this->load->view('admin/layouts/_master',$this->data); 
    }
    
    public function load_attendance()
    {
        $marked_attendance=$this->Staff_model->get_marked_attendance_staff($date=date('Y-m-d'));
         
        if(!empty($marked_attendance))
        {
            $attendance=$this->Staff_model->get_staff_list_for_attendance($date=date('Y-m-d'));
         //  echo "<pre>"; print_r($attendance);  echo $this->db->last_query();
      
            if(count($attendance) != count($marked_attendance))
            {
            ...
            {
              $attendance[$i]['marked']=$this->common->check_if_dataExist('am_staff_attendance',array("staff_id"=>$row['personal_id'],"date"=>date('Y-m-d'))); 
                //if attendance marked
                if($attendance[$i]['marked'] ==1)
                {//store present /absent
                  $attendance[$i]['attendance']=$this->common->get_name_by_id('am_staff_attendance','attendance',array("staff_id"=>$row['personal_id'],"date"=>date('Y-m-d')));   
                }
                 
                $i++;
            }
        
            $this->data['staffArr']= $attendance;
            echo $this->load->view('admin/staff_attendance_marked_sheet',$this->data, true);    
        }
        else 
        {
            $this->data['staffArr']=$this->Staff_model->get_staff_list_for_attendance($date=date('Y-m-d'));
		    echo $this->load->view('admin/staff_attendance_marking_sheet',$this->data, true);
        }  
    }
    public function load_attendance_by_date()
    {
       if($_POST)
       {
        $date=date('Y-m-d',strtotime($this->input->post('date')));
        $marked_attendance=$this->Staff_model->get_marked_attendance_staff($date);
          
        if(!empty($marked_attendance))
        {
            $attendance=$this->Staff_model->get_staff_list_for_attendance($date);
            if(count($attendance) != count($marked_attendance))
            {
                 $this->data['color']="blue";
            }
            ...
            foreach($attendance as $row)
            {
              //$attendance[$i]['marked']=$this->common->check_if_dataExist('am_staff_attendance',array("staff_id"=>$row['personal_id'],"date"=>$date)); 
				$attendance[$i]['marked']=$this->common->check_if_dataExist('am_staff_attendance',array("staff_id"=>$row['personal_id'],"date"=>$date));
                 //if attendance marked
                if($attendance[$i]['marked'] ==1)
                {//store present /absent
                  //$attendance[$i]['attendance']=$this->common->get_name_by_id('am_staff_attendance','attendance',array("staff_id"=>$row['personal_id'],"date"=>date('Y-m-d')));
					$attendance[$i]['attendance']=$this->common->get_name_by_id('am_staff_attendance','attendance',array("staff_id"=>$row['personal_id'],"date"=>$date)); 
                }
                $i++;
            }
            $this->data['staffArr']= $attendance;
        ...
        {
            $this->data['staffArr']=$this->Staff_model->get_staff_list_for_attendance($date);
		    echo $this->load->view('admin/staff_attendance_marking_sheet',$this->data, true);
        } 
       }
    }
    ...
    public function save_attendance($date = NULL)
    {
        ...
        if($_POST)
        {
             ...
             if(!empty($staff_id)) 
             {
                 ...
                 {
                    $attendance = $this->input->post('attendance_'.$row); 
                      if($attendance==1) 
                      {
                        $presant = 1 ;
                      } else {
                ...
                $data = array('staff_id'=>$row,
                             'date'=>$date,
                             'attendance'=>$presant
                             );
                $response = $this->Common_model->insert('am_staff_attendance',$data);
       ...
       if($_POST)
        {
           $marked_attendance=$this->Staff_model->get_marked_attendance_staff($date=date('Y-m-d'));
             ...
             if(!empty($att_id)) 
             {
                ...
                $exist=$this->common->check_if_dataExist('am_staff_attendance',array("staff_id"=>$row,"date"=>$date)); if($exist == 0)
                 {
                      $data = array('staff_id'=>$row,
                             'date'=>$date,
                             'attendance'=>$presant
                 ...
                 else
                 {
                      ...
                      $response = $this->Common_model->update('am_staff_attendance', $data,array("staff_id"=>$row,"date"=>date('Y-m-d')));
                     //echo $this->db->last_query();
                 }
    ...
    public function specificationCheck()
    {
        print_r($_POST);
    }
