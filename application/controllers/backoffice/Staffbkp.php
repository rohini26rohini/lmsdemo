            ...
            if($staff_personal_exist == 0){
                ...
                if($res > 0){
                   ...
                   /* $staff_personal_array=$this->staff_enrollment_model->get_staff_personaldetails_by_id($table_row_id);*/
                    ...
                    if($response_id > 0)
                    {
                        $this->db->where('personal_id', $response_id);
                        $this->db->update('am_staff_personal', array('user_id'=>$response_id));
                        $this->db->where('personal_id', $table_row_id);
                        $query = $this->db->update('am_staff_personal', array('registration_number'=>$response_id));
                        ...
                        if($query)
                        {
                            ...
                            $data.='<tr style="background:#f2f2f2">
                                    <td style="padding: 20px 0px">
                                        ...
                                        <p style="font-family: "Open Sans", sans-serif;color: #727272;font-size: 13px; padding: 0px 26px;line-height: 26px;"><b>Password :</b> '.$num.'</p>
...
*/
    ...
    function get_modules($subject_id)
    {
     ...
     function get_modules_by_subject($subject_id)
    {
    ...
    */
    ...
    { 
        ...
        if(!empty($faculty_sub_id)) {
            foreach($faculty_sub_id as $key=>$sub_id) {
                ...
                $data = array('parent_subject_id'=>$strong_subject,
                              'strong_subject_id'=>$strongmod,
                              'weak_subject_id'=>$weakmod
                             );
                $query = $this->staff_enrollment_model->update_faculty_subject($data, $sub_id);
            }
        if($query) {
        $result['status']   =   true;
        ...
        foreach($strong_subject as $subject) {
        ...
        $data = array('staff_id'=>$personal_id,
    ...
    public function staff_education_add()
    {
        if($_POST){
            ...
            //$staff_education_exist = $this->staff_enrollment_model->is_staff_education_exist($data);
            //if($staff_education_exist == 0){
                ...
                if(!empty($education_id)) {
                 foreach($education_id as $key=>$id) {
                     if($marks[$key]=="") {
                         $mark = '0.00';
                     } else {
                        $mark = $marks[$key];
                     }
                    $data = array('specification'=>$specification[$key],
                                  'school'=>$school[$key],
                                  'passing_year'=>$passing_year[$key],
                                  'marks'=>$mark[$key],
                                  'university'=>$university[$key]
                                 );
                    $res = $this->staff_enrollment_model->staff_education_edit($id, $data);
                }
                if($res = 1){
                    $what = $this->db->last_query();
                    $table_row_id = $this->db->insert_id();
                    $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
                    logcreator('update', 'database', $who, $what, $table_row_id, 'am_staff_education');
                    $staff_education_array=$this->staff_enrollment_model->get_staff_educationdetails_by_id($table_row_id);
                    $html=$table_row_id;
                }
                } else {
                foreach($category as $key=>$cat) {
                    ...
                    $data = array('personal_id'=>$edit_personal_id,
                                  'category'=>$cat,
                    ...
                    $res = $this->staff_enrollment_model->staff_education_add($data);
                }
                if($res = 1){
                    $what = $this->db->last_query();
                    $table_row_id = $this->db->insert_id();
                    $who = $this->session->userdata['user_id'].'/'.$this->session->userdata['role'];
                    logcreator('insert', 'database', $who, $what, $table_row_id, 'am_staff_education');
                    $staff_education_array=$this->staff_enrollment_model->get_staff_educationdetails_by_id($table_row_id);
                    $html=$table_row_id;
                }
                }
    ...
    //     // $this->data['staffEdit'] = $this->staff_enrollment_model->get_staff_personal_by_id($education_id);
    //     // $details = $this->data['staffEdit'];
    //     // $this->data['edit_states'] = $this->common->get_states_byCountry($details['spouse_country']);
    ...
    //                 if(isset($specificationArr[$i]) && isset($schoolArr[$i]) && isset($universityArr[$i]) && isset($passing_yearArr[$i]) && isset($marksArr[$i]))
    //                 {
    //                     // if($schoolArr[$i]!==NULL){
    //                     //     $school = $schoolArr[$i];
    //                     // }
    ...
    //                         $data= array(
    //                                     'specification'=>$specificationArr[$i],
    //                                     'category'=>$category,
    //                                     'school'=>$schoolArr[$i],
    //                                     'university'=>$universityArr[$i],
    //                                     'passing_year'=>$passing_yearArr[$i],
    //                                     'marks'=>$marksArr[$i]
    //                                 );
    //                         $result=$this->staff_enrollment_model->staff_education_edit($edit_personal_id,$data,$specificationArr[$i]);
    ...
    //                         $old=""; 
    //                         $data=
    //                         array('specification'=>$specificationArr[$i],
    //                             'marks'=>$marksArr[$i],
    //                             'category'=>$category,
    //                             'school'=>$schoolArr[$i],
    //                             'university'=>$universityArr[$i],
    //                             'passing_year'=>$passing_yearArr[$i],
    //                             'personal_id'=>$edit_personal_id
    //                         );
    ...
    //                     // if($result){
    //                     //         $ajax_response['st']=1;
    //                     //         $documents['staffArr']['personal_id'] = $edit_personal_id;
    //                     //         $documents['documents'] = $this->staff_enrollment_model->get_staff_documents($edit_personal_id);
    ...
    //                 // if($specificationArr[$i]){
    //                 if(isset($specificationArr[$i]) && isset($schoolArr[$i]) && isset($universityArr[$i]) && isset($passing_yearArr[$i]) && isset($marksArr[$i]))
    //                 {
    //                     // $specification = $specificationArr[$i];
    //                     //  if(isset($categoryArr[$i])){
    //                     //     $category = $categoryArr[$i];
    //                     // }
    //                     // if(isset($schoolArr[$i])){
    //                     //     $school = $schoolArr[$i];
    //                     // }
    //                     // if(isset($universityArr[$i])){
    //                     //     $university = $universityArr[$i];
    //                     // }
    //                     // if(isset($passing_yearArr[$i])){
    //                     //     $passing_year = $passing_yearArr[$i];
    //                     // }
    //                    if($category =='Others')
    //                    {
    ...
    //         //  $education              = $this->staff_enrollment_model->insert_staff($data);
    //         //  if ($transport_id) {
    //             $education_id       = $this->input->post('education_id');
    //             $personal_idArr     = $this->input->post('personal_id');
    //             $categoryArr        = $this->input->post('category');
    //             $specificationArr   = $this->input->post('specification');
    //             $schoolArr          = $this->input->post('school');
    //             $passing_yearArr    = $this->input->post('passing_year');
    ...
    //                      if (!empty($specificationArr[$key]) && !empty($schoolArr[$key])) {
    //                         if($categoryArr =='Others')
    //                         {
    //                           $exist=$this->staff_enrollment_model->check_specification1($personal_idArr[$key],$specificationArr[$key],$category[$key]);
    //                         }
    //                          else
    //                          {
    //                           $exist=$this->staff_enrollment_model->check_specification($personal_idArr[$key],$categoryArr[$key]);
      
    //                         }
                            
    //                          $tempArr = array(
    //                             // 'personal_id' => $personal_idArr[$key],
    //                              'specification' => $specificationArr[$key],
    //                              'category' => $categoryArr[$key],
    //                              'school' => $schoolArr[$key],
    //                              'passing_year' => $passing_yearArr[$key],
    //                              'marks' => $marksArr[$key],
    //                              'university' => $universityArr[$key]
    //                          );
    //                          array_push($educationArr, $tempArr);
    //                          $message="Add Qualification";
    //                          $action="insert";
    //                          $table_row_id=$personal_idArr[$key];
    //                          $html=$table_row_id;
                            
    //                      }else if(!empty($specificationArr[$key])){
    //                         if($exist != 0)
    //                         {
    //                         $tempArr = array(
    //                              'specification' => $specificationArr[$key],
    ...
    //                  $res = $this->staff_enrollment_model->education_add($educationArr);
    //              }else{
    //                 $res = $this->staff_enrollment_model->staff_education_edit($education_id,$educationArr);
    //              }
    //         //  }
    //          if($res = 1){
    //              $what = $this->db->last_query();
...
function send_email($type, $email, $data) {
 ...
 public function upload_documents_action()
 {
    ...
    if($_POST){
        ...
        // $spec=$this->common->get_name_by_id('am_staff_education','specification',array("education_id"=>$education_id));
        // echo '<pre>';
        // print_r($spec);
        ...
        //                  // echo "verify";
        //                    $result['st'] = 6;
        //                    $result['message'] = 'Please Verify the '.ucfirst($spec)  .'  document';
        //                    print_r(json_encode($result));
        //                    return false;
        //               }
        //        }
        ...
        //         {
        //                    $result['st'] = 6;
        //                    $result['message'] = 'Please Upload the '.ucfirst($spec)  .'  document';
        //                    print_r(json_encode($result));
        //                    return false;
        //         }
...
public function delete_faculty()
    ...
    public function fetch()
    {
        ...
        $data = $this->staff_enrollment_model->fetch_data($filter_name,$filter_role);
                        ...
                        <div class="add_wrap">
                            ...
                            $html .= '<input type="hidden" name="files" id="files" value="'.$query["file"].'" />';
                                ...
                                $html .= '<div class="col-sm-4">
                                            <div class="form-group">
                                                <label>Upload File';
                                                if(!empty($query)){ echo " : ".$query['document_name'];} 
                                        $html .= '</label>
