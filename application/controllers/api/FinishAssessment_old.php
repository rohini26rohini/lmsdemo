...
class FinishAssessment extends Direction_Controller {
  ...
  //      $exist=$this->common->check_if_dataExist('assessment',array("emp_id"=>$emp_id,"assessment_id"=>$assessment_id));
      //   if($exist == 0) {
      //     $result_id=$this->Api_model->insert_assessment($data=array('assessment_id'=>$assessment_id,
      //     'emp_id'=>$emp_id,
      //     'status'=>0
      // ));
      ...
      //   }
        ...
        foreach($request->answer_list as $row)
        { 
            //$max_assessment=$this->Api_model->get_assessment_id($emp_id);
      ...
      $result=$this->Api_model->update_grade($assessment_id, $emp_id,$attempt, array('grade'=>$grade));
